...
> - `web`'s `env_file` does not need `RUN_SCHEDULERS` set — it defaults to off/no schedulers.
> - `collector` **does** need `RUN_SCHEDULERS=inline` set at the container level — add it to the `collector` service's own `environment:` block in `docker-compose.yml` (see the repo's `docker-compose.yml` for the current form). Do not rely on the `os.environ["RUN_SCHEDULERS"] = "inline"` line at the top of `app/collector.py` to do this for you: `python -m app.collector` imports the parent `app` package first (running `app/__init__.py`, which reads `RUN_SCHEDULERS` from the environment right then) *before* `collector.py`'s own top-level code ever executes, so that in-process assignment is too late — `Config.RUN_SCHEDULERS` is already baked in as `"off"` by the time it runs. Without the compose-level `environment:` entry, the `collector` container starts cleanly, logs its normal "entering idle loop" message, and looks healthy — but runs zero background jobs (host metrics, login-metrics pruning, ADOM cache, summary job, executive summary, map cache, etc.) forever, silently. Confirm a real deployment is scheduling anything by checking that `host_metrics.db` keeps growing (`docker exec 4thealth-plus-collector python3 -c "import sqlite3,time; print(time.time() - sqlite3.connect('/app/host_metrics.db').execute('SELECT MAX(ts) FROM host_metrics').fetchone()[0])"` should stay under ~120) or that the container's thread count is well above 1 (`docker top 4thealth-plus-collector -eo pid,nlwp,cmd`).
